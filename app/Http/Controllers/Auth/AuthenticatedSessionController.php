...
class AuthenticatedSessionController extends Controller
{
    ...
    public function prepareLaunch(Request $request)
    {
        ...
        if (empty($token)) {
            Log::error(
                'Auth-Prepare: Auth Key is not set in the environment file'
            );
        ...
        if ($request->bearerToken() != $token) {
            Log::warning(
                'Auth-Prepare: Unauthorized access attempt detected. Bearer token mismatch'
            );
        ...
        if (!$name || !$refId || !is_string($name)) {
            Log::info(
                'Auth-Prepare: Validation failed. Missing or invalid parameters (name or refId)',
                [
                    'name' => $name,
                    'ref-id' => $refId,
                ]
        ...
        if (!$module) {
            Log::info(
                'Auth-Prepare: Module lookup failed. Invalid Ref ID provided',
                [
                    'refId' => $refId,
                ]
            );
    ...
    public function launch(Request $request)
    {
        ...
        if (!$key || !$name || !is_string($key) || !is_string($name)) {
            Log::info(
                'Auth-Launch: Validation failed. Missing or invalid parameters (name or key).',
                [
                    'name' => $name,
                ]
            );
        ...
        if (!$authToken) {
            Log::warning(
                'Auth-Launch: Failed authentication attempt due to unmatched token or name.',
                [
                    'name' => $name,
                ]
            );
        ...
        if (
            Carbon::parse($authToken->created_at)->lt(
                now()->subSeconds($expireAfter)
            )
        ) {
            Log::info('Auth-Launch: Token session expired', [
                'tokenCreatedAt' => $authToken->created_at,
        ...
        Log::info(
            'Auth-Launch: Authentication successful. Logging in user with name {name}',
            [
                'name' => $name,
            ]
        );
